...
| E2E-MCP-MARKET-SEMANTICS | Registry 记录映射为安装模板时保留包版本、named/positional runtime/package 参数与 required/optional 环境变量语义；远端 header 变量同时识别注册表的 `{name}` 与目录的 `${NAME}` 两种写法，会为它索取值，并将其代入实际发出的 header 而不是作为字面量发出（ADR registry-header-variable-spelling） | 确定性映射断言 |
